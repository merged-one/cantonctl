 ...
 *
 * Also provides `detectTopology()` to discover a running multi-node topology
 * from the `.cantonctl/` directory generated by `dev --full`.
 *
 * @example
 * ```ts
 * import { generateTopology, detectTopology } from './topology.js'
 *
 ...
 * })
 *
 * // Detect existing topology from .cantonctl/ directory
 * const existing = await detectTopology('/path/to/project')
 * ```
 */
...
// ---------------------------------------------------------------------------
// Topology detection — reads .cantonctl/ to discover a running multi-node topology
// ---------------------------------------------------------------------------

const CONFIG_DIR_NAME = '.cantonctl'

/**
 * Detect a running multi-node topology by reading `.cantonctl/docker-compose.yml`.
 * Returns the topology metadata or null if no multi-node topology is configured.
 */
export async function detectTopology(projectDir: string): Promise<GeneratedTopology | null> {
  try {
    const confPath = path.join(projectDir, CONFIG_DIR_NAME, 'canton.conf')
    const composePath = path.join(projectDir, CONFIG_DIR_NAME, 'docker-compose.yml')
