...
describe('CI parity manifest', () => {
  ...
  it('keeps package scripts aligned with the manifest-driven runner', () => {
    ...
    expect(packageJson.scripts.test).toBe('vitest run --project unit --project unit-commands --project ui')
    expect(packageJson.scripts['test:unit']).toBe('vitest run --project unit --project unit-commands --project ui')
    expect(packageJson.scripts['test:watch']).toBe('vitest --project unit --project unit-commands --project ui')
    expect(packageJson.scripts['test:coverage']).toBe(
      'node scripts/verify-coverage-exclusions.mjs && vitest run --project unit --project unit-commands --project ui --coverage',
    )
    ...
    expect(packageJson.scripts['test:coverage:strict']).toBe(
      'node scripts/verify-coverage-exclusions.mjs && COVERAGE_STRICT=1 vitest run --project unit --project unit-commands --project ui --coverage',
    )
