...
export default class LocalnetStatus extends Command {
  static override description = 'Show status for an upstream Splice LocalNet workspace'

  static override examples = [
    '<%= config.bin %> localnet status --workspace ../quickstart',
    '<%= config.bin %> localnet status --workspace ../quickstart --json',
  ]
  ...
  async run(): Promise<void> {
    const {flags} = await this.parse(LocalnetStatus)
    const out = createOutput({json: flags.json})

    try {
      const result = await this.createLocalnet().status({workspace: flags.workspace})
