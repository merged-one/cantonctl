...
describe('runtime-heavy command surface', () => {
  ...
  it('deploy normalizes missing multi-node package ids to null', async () => {
    class TestDeploy extends Deploy {
      protected override createBuilder(): Builder {
        return {
          build: vi.fn(),
    ...
    expect(json.data).toEqual(expect.objectContaining({
      participants: [
        {mainPackageId: null, participant: 'participant1', port: 7575},
        {mainPackageId: null, participant: 'participant2', port: 7576},
      ],
    }))
  })
  ...
  it('deploy defaults to the local network when no argument is provided', async () => {
    const deploy = vi.fn().mockResolvedValue({
      darPath: '/repo/.daml/dist/demo.dar',
      dryRun: false,
      durationMs: 35,
    ...
    const result = await captureOutput(() => TestDeploy.run(['--json'], {root: CLI_ROOT}))
    expect(result.error).toBeUndefined()
    expect(deploy).toHaveBeenCalledWith(expect.objectContaining({network: 'local'}))

    const json = parseJson(result.stdout)
    expect(json.success).toBe(true)
    expect(json.data).toEqual(expect.objectContaining({network: 'local'}))
  })
  ...
  it('serve stops the managed sandbox on shutdown when it started one', async () => {
    const sandboxStart = vi.fn().mockResolvedValue(undefined)
    ...
    const result = await captureOutput(() => TestServe.run([], {root: CLI_ROOT}))
    expect(result.error).toBeUndefined()
    expect(sandboxStart).toHaveBeenCalledOnce()
  ...
  it('playground uses a remote profile without starting a managed sandbox', async () => {
    const sandboxStart = vi.fn()
    const serverStart = vi.fn().mockResolvedValue(undefined)
    ...
    const result = await captureOutput(() => TestPlayground.run(['--profile', 'splice-devnet', '--no-open'], {root: CLI_ROOT}))
    expect(result.error).toBeUndefined()
    expect(sandboxStart).not.toHaveBeenCalled()
    expect(serverStart).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('playground skips the remote-profile banner when sandbox startup is explicitly disabled', async () => {
    ...
    const result = await captureOutput(() => TestPlayground.run(['--profile', 'splice-devnet', '--no-open', '--no-sandbox'], {root: CLI_ROOT}))
    expect(result.error).toBeUndefined()
    expect(result.stdout).not.toContain('Using profile "splice-devnet" without starting a managed sandbox')
  })
  ...
  it('playground stops the managed sandbox on shutdown in sandbox mode', async () => {
    const sandboxStop = vi.fn().mockResolvedValue(undefined)
    const serverStop = vi.fn().mockResolvedValue(undefined)
  ...
  it('console reports missing networks through CantonctlError formatting', async () => {
    class TestConsole extends Console {
      protected override async loadProjectConfig(): Promise<CantonctlConfig> {
        return createConfig()
      }
    ...
    const result = await captureOutput(() => TestConsole.run(['--network', 'missing'], {root: CLI_ROOT}))
    expect(result.error).toBeDefined()
    expect(result.stderr).toContain('Network "missing" not found in cantonctl.yaml')
    expect(result.stderr).toContain('Available: devnet, local')
  })

  it('console reports missing networks when no networks are configured', async () => {
    class TestConsole extends Console {
      protected override async loadProjectConfig(): Promise<CantonctlConfig> {
  ...
  it('console defaults to the first participant and admin auth when no parties are configured', async () => {
    const createSandboxToken = vi.fn().mockResolvedValue('sandbox-token')
    const execute = vi.fn().mockResolvedValue(false)

    class TestConsole extends Console {
      protected override createCompleter(options: {partyNames: string[]}): Completer {
        expect(options.partyNames).toEqual([])
        return {
          complete: vi.fn().mockReturnValue([['help'], 'hel']),
        }
      }

      protected override createExecutor(options: any): Executor {
        expect(options.defaultParty).toBeUndefined()
        return {execute}
      }

      protected override createLedgerClient(options: {baseUrl: string; token: string}): LedgerClient {
        expect(options.baseUrl).toBe('http://localhost:7575')
      ...
      protected override createReadlineInterface(options: any) {
        expect(options.completer('hel')).toEqual([['help'], 'hel'])
        return {
          [Symbol.asyncIterator]: async function* () {
            yield 'exit'
          },
      ...
      protected override async createSandboxToken(options: Parameters<Console['createSandboxToken']>[0]): Promise<string> {
    ...
    const result = await captureOutput(() => TestConsole.run([], {root: CLI_ROOT}))
    expect(result.error).toBeUndefined()
    expect(createSandboxToken).toHaveBeenCalledOnce()
    expect(execute).toHaveBeenCalledOnce()
    expect(result.stdout).toContain('Connected to local → participant1 at http://localhost:7575')
  })

  it('console connects directly to remote networks without topology detection', async () => {
    const detectProjectTopology = vi.fn()
    const execute = vi.fn()
    ...
    class TestConsole extends Console {
      ...
      protected override createLedgerClient(options: {baseUrl: string; token: string}): LedgerClient {
        expect(options.baseUrl).toBe('https://ledger.example.com')
        return {
          allocateParty: vi.fn(),
          getActiveContracts: vi.fn(),
          getLedgerEnd: vi.fn(),
    ...
    const result = await captureOutput(() => TestConsole.run(['--network', 'devnet'], {root: CLI_ROOT}))
    expect(result.error).toBeUndefined()
    expect(detectProjectTopology).not.toHaveBeenCalled()
    expect(execute).toHaveBeenCalledOnce()
    expect(result.stdout).toContain('Connected to devnet at https://ledger.example.com')
  })

  it('console reports repl command failures and continues prompting', async () => {
    const prompt = vi.fn()
    const close = vi.fn()
    ...
    class TestConsole extends Console {
      ...
      protected override createExecutor(): Executor {
        return {
          execute: vi.fn()
            .mockRejectedValueOnce(new CantonctlError(ErrorCode.CONSOLE_UNKNOWN_COMMAND, {
              suggestion: 'Try the help command.',
            }))
    ...
    expect(result.stderr).toContain(`${ErrorCode.CONSOLE_UNKNOWN_COMMAND}:`)
    expect(result.stdout).toContain('Try the help command.')
    expect(result.stderr).toContain('Error: boom')
  })

  it('console continues when the executor asks to keep the repl open', async () => {
    const execute = vi.fn()
  ...
  it('console omits blank suggestions from repl errors', async () => {
    class TestConsole extends Console {
      protected override createCompleter(): Completer {
        return {
          complete: vi.fn().mockReturnValue([[], '']),
      ...
      protected override createExecutor(): Executor {
        return {
          execute: vi.fn()
            .mockRejectedValueOnce(new CantonctlError(ErrorCode.CONSOLE_PARSE_ERROR))
            .mockResolvedValueOnce(false),
        }
      }
    ...
    const result = await captureOutput(() => TestConsole.run([], {root: CLI_ROOT}))
    expect(result.error).toBeUndefined()
    expect(result.stderr).toContain(`${ErrorCode.CONSOLE_PARSE_ERROR}:`)
  ...
  it('console rethrows unexpected startup failures', async () => {
    class TestConsole extends Console {
      protected override async loadProjectConfig(): Promise<CantonctlConfig> {
        throw new Error('boom')
      }
    ...
    await expect(TestConsole.run([], {root: CLI_ROOT})).rejects.toThrow('boom')
  })
})
